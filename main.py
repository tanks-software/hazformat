...
class GoogleDriveSheets:
    def __init__(self, service_account_info):
        # Write service_account_info dict to temp file for credentials
...
def load_master_data_from_drive(service_account_info, spreadsheet_id):
    ...
    ports = ports_df["Port"].dropna().tolist()
    vessels = vessels_df["Vessel_Name"].dropna().tolist() if not vessels_df.empty else []

    return cargo_data, shippers, shipper_contacts, consignees, consignee_addresses, ports, equipment_types, vessels
...
def main():
    ...
    cargo_data_raw, shippers, shipper_contacts, consignees, consignee_addresses, ports, equipment_types, vessels = load_master_data_from_drive(service_account_info, MASTER_SPREADSHEET_ID)
    ...
    # UI for selecting cargo and autofilling details
